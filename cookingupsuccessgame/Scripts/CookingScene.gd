...
# Ingredient spawning
var pepperoni_scene = preload("res://Scenes/pepperoni.tscn")
var cheese_scene = preload("res://Scenes/cheese.tscn")
var mushrooms_scene = preload("res://Scenes/mushrooms.tscn")
var sauce_scene = preload("res://Scenes/sauce.tscn")

# Spawn positions for each ingredient
var pepperoni_spawn_area = Vector2(-475.5, -557.5)
var cheese_spawn_area = Vector2(-475.5, -485.5)
var mushrooms_spawn_area = Vector2(-475.5, -413.5)
var sauce_spawn_area = Vector2(-475.5, -335.5)
var ingredient_spacing = Vector2(50, 0)  # Space between ingredients
...
func spawn_pepperoni():
	"""Spawn pepperoni instances based on global pepperoni count"""
	var pepperoni_count = Global.ingredients[0]  # Index 0 is pepperoni
	...
	for i in range(pepperoni_count):
		var new_pepperoni = pepperoni_scene.instantiate()
		new_pepperoni.name = "pepperoni_" + str(i)
		
		# Calculate position - arrange in a grid
		var row = i / 5  # 5 pepperoni per row
		var col = i % 5
		var spawn_pos = pepperoni_spawn_area + Vector2(col * ingredient_spacing.x, row * ingredient_spacing.y)
...
func spawn_cheese():
	...
	var cheese_count = Global.ingredients[1]  # Index 1 is cheese
	...
	for i in range(cheese_count):
		...
		# Calculate position - arrange in a grid
		var row = i / 5  # 5 cheese per row
		var col = i % 5
		var spawn_pos = cheese_spawn_area + Vector2(col * ingredient_spacing.x, row * ingredient_spacing.y)
...
func spawn_mushrooms():
	...
	var mushrooms_count = Global.ingredients[2]  # Index 2 is mushrooms
	...
	for i in range(mushrooms_count):
		var new_mushrooms = mushrooms_scene.instantiate()
		new_mushrooms.name = "mushrooms_" + str(i)
		
		# Calculate position - arrange in a grid
		var row = i / 5  # 5 mushrooms per row
		var col = i % 5
		var spawn_pos = mushrooms_spawn_area + Vector2(col * ingredient_spacing.x, row * ingredient_spacing.y)
...
func spawn_sauce():
	...
	var sauce_count = Global.ingredients[3]  # Index 3 is sauce
	...
	for i in range(sauce_count):
		var new_sauce = sauce_scene.instantiate()
		new_sauce.name = "sauce_" + str(i)
		
		# Calculate position - arrange in a grid
		var row = i / 5  # 5 sauce per row
		var col = i % 5
		var spawn_pos = sauce_spawn_area + Vector2(col * ingredient_spacing.x, row * ingredient_spacing.y)
