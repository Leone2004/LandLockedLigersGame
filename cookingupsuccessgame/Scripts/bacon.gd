	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT:
			if event.pressed and not is_dragging and not is_attached_to_pizza:
				...
				if _is_mouse_over_bacon(local_mouse_pos):
					is_dragging = true
...
func _is_mouse_over_bacon(local_mouse_pos: Vector2) -> bool:
...
func attach_to_pizza(pizza: Node) -> void:
	...
	# Decrease global bacon count (index 6 for bacon)
	if Global.ingredients.size() > 6:
		Global.ingredients[6] -= 1
	reparent(pizza)
