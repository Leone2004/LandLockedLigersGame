	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT:
			if event.pressed and not is_dragging and not is_attached_to_pizza:
				...
				if _is_mouse_over_onion(local_mouse_pos):
					is_dragging = true
...
func _is_mouse_over_onion(local_mouse_pos: Vector2) -> bool:
...
func attach_to_pizza(pizza: Node) -> void:
	...
	# Decrease global onion count (index 12 for onion)
	if Global.ingredients.size() > 12:
		Global.ingredients[12] -= 1
	reparent(pizza)
